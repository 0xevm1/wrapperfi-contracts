...
describe("CandyWrapper", function () {
    ...
    beforeEach(async function () {
        ...
        await candyWrapper.setAuctionSaleStartTime(1681519654);
    });
    ...
    describe("TokenURI", async function () {
       it( "Mint 1 NFT & Display Unrevealed URI and then Revealed Uri", async function () {
           ...
           const amount: BigNumber = ethers.utils.parseEther((1 * quantity).toString());
            ...
            await candyWrapper.mint(0, quantity,{value: amount}).then();
        ...
        it("Mint 5 SVGs", async function () {
            ...
            await candyWrapper.mint(0, quantity, {value: amount});
        ...
        it("Mint 20 SVGs between 4 addresses, one mints 6 SVGs which is too many and should fail, another mints 6 betwen two transactions and the second should fail, reveal, update DAO list, check", async function () {
            ...
            await candyWrapper.connect(addr1).mint(0, quantity, {value: amount});
            ...
            await expect(candyWrapper.connect(addr2).mint(0, quantity, {value: amount})).to.be.revertedWith("Too much Candy.");
            ...
            quantity = 3;
            amount = ethers.utils.parseEther(((1 * quantity).toString()));
            await candyWrapper.connect(addrs[0]).mint(0, quantity, {value: amount});
            balance = await candyWrapper.balanceOf(addrs[0].address);
            //console.log("minter balance: ", balance);
            await expect(balance).to.equal(BigNumber.from(quantity));
            quantity = 3;
            amount = ethers.utils.parseEther(((1 * quantity).toString()));
            await expect(candyWrapper.connect(addrs[0]).mint(0, quantity, {value: amount})).to.be.revertedWith("Too much Candy.");
            balance = await candyWrapper.balanceOf(addrs[0].address);
            await expect(balance).to.equal(BigNumber.from(quantity));
            ...
            quantity = 5;
            amount = ethers.utils.parseEther((1 * quantity).toString());
            await candyWrapper.connect(addrs[1]).mint(0, quantity, {value: amount});
        ...
        it("set allowlist for 2000 addresses", async function () {
            ...
            await candyWrapper.seedAllowlist(addresses, {
                gasLimit: 3000000,
            });
        });
    });
